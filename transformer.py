...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=256, nhead=8, num_encoder_layers=6,
                 ...
                 num_feature_levels=4, activation="relu", normalize_before=False,
    ...
    def forward(self, srcs, masks, query_embeds, pos_embeds):
        ...
        bs, _, c = memory.shape
        query_embed = query_embeds.unsqueeze(0).expand(bs, -1, -1)
        ...
        if self.training:
            ...
            hs, maps = self.decoder(tgt, memory, memory_key_padding_mask=mask_flatten,
                                    pos=lvl_pos_embed_flatten, query_pos=query_embed)
            return hs.squeeze(), maps, level_start_index, spatial_shapes
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        if self.training:
            ...
            intermediate_map = []
            for layer in self.layers:
                output, maps = layer(output, memory, tgt_mask=tgt_mask,
                                     memory_mask=memory_mask,
                                     tgt_key_padding_mask=tgt_key_padding_mask,
                                     memory_key_padding_mask=memory_key_padding_mask,
                                     pos=pos, query_pos=query_pos)
